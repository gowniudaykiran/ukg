...
def create_user(sender, *args, **kwargs):
    if kwargs['created']:
        user = User.objects.create(username=kwargs['instance'],password="dummypass",email=kwargs['instance'].email)
        if kwargs['instance'].is_lecturer:
...
#roll_no is used for identifying students uniquely
